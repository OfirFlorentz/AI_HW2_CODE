...
class OrderedAlphaBetaPlayer(AlphaBetaPlayer):
    ...
    def rb_minmax(self, depth, time_left, board, my_turn=True, alpha=-2, beta=2, path=""):
        def hash256(word):
        ...
        if my_turn == 1:
            ...
            prev_loc = self.loc
            board[prev_loc] = -1
            self.directions.sort(key = lambda d: self.score_dict[hash256(path+str(d[0])+str(d[1]))] if hash256(path+str(d[0])+str(d[1])) in self.score_dict else 0, reverse = True)
            for d in self.directions:
                ...
                if 0 <= i < len(board) and 0 <= j < len(board[0]) and board[i][j] == 0:  # then move is legal
                    ...
                    self.loc = new_loc
                    self.available -= 1
                    _, score, _, _ = self.rb_minmax(depth-1,  time_left - _time() + start, board, 1-my_turn, alpha, beta, path+str(d[0])+str(d[1]))
                    self.available += 1
                    assert self.count_players(board) == (1,1)
                    board[new_loc] = 0
            ...
            self.score_dict[hash256(path)]=min_score
            return best_move, min_score, best_new_loc, self.score_dict
